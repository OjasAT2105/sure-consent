...
const Design = () => {
  ...
  const designItems = complianceLaw.name === 'CCPA' 
    ? allDesignItems.filter(item => item.id === 'cookie-bar' || item.id === 'accept-btn')
    : allDesignItems;
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-semibold mb-2" style={{fontSize: '20px', color: '#111827'}}>Design Settings</h1>
        <p className="" style={{fontSize: '14px', color: '#4b5563'}}>Customize the appearance and styling of your cookie banner</p>
      </div>
              ...
              {item.id === 'accept-all-btn' && (
                <label className="flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) => updateSetting('accept_all_enabled', e.target.checked)}
                    className="sr-only"
                  />
                  <div className={`relative w-11 h-6 rounded-full transition-colors ${acceptAllEnabled ? 'bg-blue-600' : 'bg-gray-300'}`}>
                    <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${acceptAllEnabled ? 'translate-x-5' : 'translate-x-0'}`}></div>
                  </div>
                </label>
              )}
              <Button
        ...
        >
          ...
          <Dialog.Panel className="max-w-2xl w-full mx-auto rounded-none">
            <Dialog.Body className="p-4">
              <BannerDesign activeTab={item.activeTab} closePopup={closePopup} />
            </Dialog.Body>
          </Dialog.Panel>
        </Dialog>
      ))}
