...
const ButtonOrder = () => {
  ...
  const [buttonOrder, setButtonOrder] = useState(['decline', 'accept', 'accept_all']);
  const [draggedItem, setDraggedItem] = useState(null);

  useEffect(() => {
    const savedOrder = getCurrentValue('button_order') || 'decline,accept,accept_all';
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-600 mb-4">Drag and drop to reorder buttons</p>
        ...
        {buttonOrder.map((button, index) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium text-gray-800">{buttonLabels[button]}</span>
              <span className="text-sm text-gray-500 ml-auto">Position {index + 1}</span>
            </div>
          </div>
        ))}
      </div>
